...
async function fetchData(iataCode) {
  const options = {
    ...
    headers: {
      'X-RapidAPI-Key': 'e10ffa8f98msh9c5812703e2b88cp171d7djsn46ca4a3a39fe',
...
const searchFunc = event => {
  ...
  if (inputIata) {
    fetchData(inputIata).then(
      data => {

        iataCode.innerHTML = data.iata;
        Name.innerHTML = data.name;
        countryLocation.innerHTML = data.location;
        link.href = data.website;
      
        stNo.innerHTML = data.street_number;
        st.innerHTML = data.street;
        state.innerHTML = data.state;
        city.innerHTML = data.city;
        country.innerHTML = data.country;
        postalCode.innerHTML = data.postal_code;
        phn.innerHTML = data.phone;
